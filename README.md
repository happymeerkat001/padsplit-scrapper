...
Sifely lock-code automation (`padsplit_scraper/lock_codes.py`) covers Ridge Oak and Pebble Shores only; every other house is a no-op. It is Mac morning/afternoon only. Default off until Mac `.env` sets `LOCK_CODES_ENABLE=1` (and `PADSPLIT_ENABLE_ACTION_HOOKS=1`, same as lockout/leak). GitHub Actions / CI must not rotate locks or post Discord. New move-in sets that room lock to the last four digits of the tenant phone, messages the tenant on PadSplit (digits allowed on the member thread only), and updates Firestore `property_codes` for `codes.html`. Missing Firebase service account is fail-closed Need-you / skip — do not invent credentials. Move-out or member terminated resets the vacated room lock to `VACANT_ROOM_DEFAULT` (no in-repo default; if that env var is missing the reset is skipped) and updates the codes page (does not wait for Ang on the room), then always asks Ang on `#ai-automations` whether to change front/back door keycodes for that house (Discord: house/room/member only, never digits). Only a reply from `ANG_DISCORD_USER_ID` can approve; if that env var is unset, no reply is an approval. After Ang yes: rotate front/back, update the codes page, and PadSplit-blast remaining tenants at that house with the new door codes (digits allowed on PadSplit only). After Ang no: leave front/back alone; the room is already the vacant default. Outbound Discord never includes lock-code digits, phone digits, or the vacant default. Missing `SIFELY_API_KEY` is a Need-you no-op. lockout_reply still obtains the Spanish Moss back door through this module.
