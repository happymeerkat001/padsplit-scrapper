...
Sifely lock-code automation (`padsplit_scraper/lock_codes.py`) is Mac morning/afternoon only. Default off until Mac `.env` sets `LOCK_CODES_ENABLE=1` (and `PADSPLIT_ENABLE_ACTION_HOOKS=1`, same as lockout/leak). GitHub Actions / CI must not rotate locks or post Discord. New move-in sets that room lock to the last four digits of the tenant phone, messages the tenant on PadSplit (digits allowed on the member thread only), and updates Firestore `property_codes` for `codes.html`. Missing Firebase service account is fail-closed Need-you / skip — do not invent credentials. Move-out and termination signals only create an approval request. Ang or Joe must directly reply `confirm vacant room reset` to that specific house/room request before the vacant room is reset. Configure verified Discord IDs as `LOCK_CODES_APPROVER_USER_ID` (Ang) and `LOCK_CODES_JOE_USER_ID` (Joe). Don, generic replies, dates and payment signals do not authorize resets. Shared doors separately require Ang's direct yes/no reply. Occupied-room/manual changes are not inferred by this workflow. Completed lock, Firestore and per-member delivery stages are checkpointed without raw passcodes; failed stages remain pending. Interrupted lock effects require matching readback; uncertain message delivery requires manual reconciliation to avoid duplicates. Keep one executor active and reconcile the existing pilot before enablement. Outbound Discord never includes lock-code digits, phone digits, or the vacant default. Missing `SIFELY_API_KEY` is a Need-you no-op. lockout_reply still obtains the Spanish Moss back door through this module.
