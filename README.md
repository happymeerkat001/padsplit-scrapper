...
Water-leak auto-reply (`padsplit_scraper/leak_reply.py`) detects a current member leak/flood report at 100% precision and SENDS a shut-off pack on that PadSplit thread (call/text Quo + water-key YouTube + meter-to-house shut-off copy). It also posts a digit-free `WATER_KEY_ORDER` event to Discord `#ai-automations` (or `DISCORD_AUTOMATIONS_CHANNEL_ID`) for Cart. No Amazon purchase in this scraper. Host reminder blasts and historical “previous leak” chatter do not fire. Default off until Mac `.env` sets `LEAK_REPLY_ENABLE=1`. CI must not send. Discord posts never include lock codes or any digits.
