...
- `padsplit_scraper/leak_reply.py` — member water-leak auto-SEND on PadSplit at 100% current-leak precision. Body is a 1:1 adaptation of Firestore `templates/shared` t5 (Water leak announcement) plus Quo; baked t5 fallback. Emits digit-free `WATER_KEY_ORDER` on `#ai-automations` for Cart. No Amazon purchase here. Default off until Mac `.env` sets `LEAK_REPLY_ENABLE=1`. CI must not send. Discord posts never include codes or digits.
