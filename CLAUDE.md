...
- `padsplit_scraper/leak_reply.py` — member water-leak auto-SEND on PadSplit for active water emergencies only (burst / pipe / main / flooding / wall-ceiling). Toilet-only slow leak/clog/wax-ring does not fire. Body is a 1:1 adaptation of Firestore `templates/shared` t5 (Water leak announcement) plus Quo; baked t5 fallback. Emits `WATER_KEY_ORDER` on `#ai-automations` for Cart (house + full ship-to; digit-free except address). No Amazon purchase here. Default off until Mac `.env` sets `LEAK_REPLY_ENABLE=1`. CI must not send. Discord posts never include codes. Nest interim-handles until enable is live.
