...
- `padsplit_scraper/lock_codes.py` — Sifely Open API lock-codes for Ridge Oak and Pebble Shores only (every other house is a no-op). Move-in sets the room to the tenant phone last-four; move-out/terminate resets that room to the vacant default immediately; shared front/back doors change only after an explicit Ang yes on `#ai-automations`. Mac morning/afternoon only; default off until `LOCK_CODES_ENABLE=1`. CI must not rotate or post. Discord is digit-free. Sifely application errors fail closed and retries do not rotate a lock twice. lockout_reply still reads the Spanish Moss back door through this module and does not rotate it from the lockout path.
