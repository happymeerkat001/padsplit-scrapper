...
./run_field_mms.sh    # Don+Dad+Ang GV Quo 1:1 SMS blast (morning 7am CT only; skip if both sources empty)
...
- Don-field Quo SMS blast is launchd-managed by `com.padsplit.field-mms`
  ...
  First send is the next 7:00am CT slot. Quo 1:1 to Don + Dad + Ang GV (`FIELD_MMS_QUO_TO`
...
- `padsplit_scraper/field_mms.py` — morning 7am CT Don+Dad+Ang GV Quo 1:1 SMS blast (PadSplit host inbox + Discord #ai-tasks-temp; no Joe). Sends via Quo SMS (`QUO_API_KEY`) first (one POST per recipient), then `google_voice_chrome.py` (Mac Chrome / Voice), then Messages `Don Field`.
...
# FIELD_MMS_QUO_TO=        # optional Quo 1:1 list; default Don +1 (214) 779-8338, Dad +1 (945) 241-3070, Ang GV +1 (469) 626-7260 (no Joe)
