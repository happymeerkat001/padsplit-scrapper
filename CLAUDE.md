...
- Don-field Quo SMS blast is launchd-managed by `com.padsplit.field-mms`
  ...
  First send is the next 7:00am CT slot. Quo one group send to Don + Dad +
  Ang GV (`FIELD_MMS_QUO_TO` override). GV / Messages `Don Field` group is
  fallback only. CI must not send.
  ...
  the Mac; it does not have the Google Voice unfamiliar-IP challenge. Fallbacks
  are Google Voice group SMS from Ang’s Mac Chrome session (`google_voice`;
  Playwright + `FIELD_MMS_CHROME_USER_DATA_DIR` / optional
  `FIELD_MMS_CHROME_PROFILE_DIRECTORY`, default the mr.angli Chrome profile
  already signed into Voice 469) then the Messages.app chat named exactly
  `Don Field` unless transport is strict `quo` or `google_voice`.
  `FIELD_MMS_TRANSPORT=messages` forces Messages-only. CI must not send.
  If Google challenges, use the Messages `Don Field` group. Never paste Quo
  keys or Google passwords into the repo.
...
- `padsplit_scraper/lock_codes.py` — Sifely Open API lock-codes (move-in last-four; move-out room reset only after explicit Ang/Joe confirmation + separate Ang-only doors yes/no + PadSplit housemate blast on yes). Mac morning/afternoon only; default off until `LOCK_CODES_ENABLE=1`. CI must not rotate or post. Discord `#ai-automations` is digit-free. lockout_reply still obtains Spanish Moss back via this module.
...
verified Discord user ID of Ang. `LOCK_CODES_JOE_USER_ID` is the independently verified
Joe ID, authorized only for vacant-room resets. Direct reply `confirm vacant room reset`
to the event-specific house/room request confirms physical vacancy and authorizes
that reset. Ang separately replies yes/no to authorize shared doors. Don, generic
replies, scheduled dates, termination and payment signals never authorize room resets.
Occupied-room/manual changes are outside this automatic move-out path.
Missing configuration, other authors, bots,
...
# FIELD_MMS_QUO_TO=        # optional Quo group list; default Don +1 (214) 779-8338, Dad +1 (945) 241-3070, Ang GV +1 (469) 626-7260 (no Joe)
