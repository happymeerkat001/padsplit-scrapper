...
- `padsplit_scraper/lock_codes.py` — Sifely Open API lock-codes (move-in last-four; move-out room vacant-default immediately + Ang doors-only yes/no + PadSplit housemate blast on yes). Mac morning/afternoon only; default off until `LOCK_CODES_ENABLE=1`. CI must not rotate or post. Discord `#ai-automations` is digit-free. lockout_reply still obtains Spanish Moss back via this module.
