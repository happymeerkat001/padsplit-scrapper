...
class FieldMmsQuoTests(unittest.TestCase):
    ...
    def test_send_via_quo_posts_once_with_all_recipients(self) -> None:
        posted: list[dict] = []
        ...
        self.assertEqual(call["json"]["to"], [DON_PHONE, DAD_PHONE, ANG_GV_PHONE])
        self.assertNotIn(JOE_PHONE, call["json"]["to"])
        self.assertEqual(len(call["json"]["to"]), 3)
    ...
    def test_send_via_quo_uses_requests_post(self) -> None:
        ...
        self.assertEqual(payload["to"], [DON_PHONE, DAD_PHONE, ANG_GV_PHONE])
        self.assertNotIn(JOE_PHONE, payload["to"])
