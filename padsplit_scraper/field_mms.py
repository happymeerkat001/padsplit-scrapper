...
and Ang GV (one POST per recipient). Fallbacks are Google Voice group SMS
(Ang's Mac Chrome) then the Messages.app chat named exactly "Don Field".
Prefer the Mac launchd job; Quo HTTP does not need a residential IP the
way Google Voice does.
"""
...
# Quo blast defaults: Don + Dad + Ang GV 1:1 SMS (not group). Joe is not
# on this list. Override with FIELD_MMS_QUO_TO.
...
def send_via_quo(
    ...
    *,
    ...
    uses one POST per recipient so Don, Dad, and Ang GV each get a 1:1, not a
    new group thread. GV / Messages still use the existing Don Field group
    as fallback.
    """
