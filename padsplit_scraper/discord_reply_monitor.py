...
def _fetch_replies(token: str, channel: str, message_id: str) -> List[Dict]:
    ...
    # Discord returns newest-first; process oldest-first to match prior Slack assumption.
...
def main() -> None:
    ...
    with create_session() as session:
        ...
        for reply in replies:
            reply_ts = str(reply.get("id") or "")
            if not reply_ts or reply_ts == str(meta["message_id"]):
                continue
