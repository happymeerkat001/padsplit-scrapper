...
def _address_variants(address: str) -> List[str]:
    ...
    return [v for v in {raw, compact} if v]


def _build_address_task_index(tasks: Dict[str, List[Dict]]) -> Dict[str, List[int]]:
    ...
    for bucket in ("Requests", "Open"):
        for task in tasks.get(bucket) or []:
            ...
            for variant in _address_variants(combined) + _address_variants(address):
                index.setdefault(variant, []).append(int(task_id))
    return index
...
def _find_matching_tasks(text: str, address_index: Dict[str, List[int]]) -> Tuple[str, List[int]]:
    lowered = text.lower()
    normalized = re.sub(r"[^a-z0-9]+", " ", lowered).strip()
    for known_address, task_ids in address_index.items():
...
def main() -> None:
    ...
    address_index = _build_address_task_index(tasks)
    ...
    with create_session() as session:
        ...
        for reply in replies:
            ...
            if COMPLETE_RE.search(text):
                _, task_ids = _find_matching_tasks(text, address_index)
