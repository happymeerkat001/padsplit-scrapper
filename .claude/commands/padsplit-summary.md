...
./venv/bin/python3 slack_notifier.py
```

- `message_summarizer.py` sends latest messages to MiniMax AI, posts urgent summary to `SLACK_WEBHOOK_MESSAGES`.
- `slack_notifier.py` fetches DFW weather + formats tasks digest, posts to `SLACK_WEBHOOK_TASKS`. Weather only included during 5-9 CT.
